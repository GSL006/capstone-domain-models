...
class HierarchicalTechnologyPaperDataset(Dataset):
    ...
    def _extract_sections(self, text):
        """Extract sections from the technology paper text"""
        # Technology-specific section markers
        section_markers = [
            'abstract', 'introduction', 'related work', 'background', 
            'system architecture', 'implementation', 'methodology', 'algorithms',
            'experimental setup', 'evaluation', 'results', 'performance',
...
# Enhanced feature extractor for technology papers
class TechnologyBiasFeatureExtractor:
    """Extract features that might indicate bias in technology research papers"""
    
    def __init__(self):
        # Basic patterns
        self.p_value_pattern = r'p\s*[<>=]\s*0\.0\d+'
        self.significance_stars = r'\*{1,3}\s*'
        self.accuracy_pattern = r'(?:accuracy|acc|precision|recall|f1)\s*[=:]\s*[-+]?[0-9]*\.?[0-9]+%?'
        self.percentage_pattern = r'\d+(?:\.\d+)?%'
        
        # Technology-specific hedge words
        self.hedge_words = [
            ...
            'preliminary', 'experimental', 'exploratory'
        ]
        
        # Technology-specific certainty words
        self.certainty_words = [
            ...
            'state-of-the-art', 'groundbreaking', 'revolutionary', 'breakthrough',
            'superior', 'optimal', 'best', 'outperforms'
        ]
        
        # Technology theory and framework references
        self.theory_terms = [
            'deep learning', 'neural network', 'machine learning', 'artificial intelligence',
            'algorithm', 'framework', 'architecture', 'model', 'system design',
            'distributed system', 'cloud computing', 'edge computing', 'blockchain',
            'quantum computing', 'cryptography', 'protocol', 'api design'
        ]
        
        # Technology-specific claim words
        self.claim_terms = [
            'improve', 'increase', 'decrease', 'enhance', 'optimize', 'accelerate',
            'scale', 'performance', 'efficiency', 'throughput', 'latency',
            'accuracy', 'precision', 'speed', 'scalability', 'reliability',
            'bandwidth', 'memory usage', 'computational cost', 'runtime'
        ]
        
        # Technology jargon that might signal overconfidence
        self.tech_jargon = [
            'cutting-edge', 'next-generation', 'disruptive', 'innovative',
            'state-of-the-art', 'bleeding-edge', 'paradigm shift', 'game-changing',
            'revolutionary', 'transformative', 'industry-leading', 'world-class',
            'enterprise-grade', 'web-scale', 'planet-scale'
        ]
        
        # Methodology and validation terms
        self.method_terms = [
            'experiment', 'benchmark', 'dataset', 'baseline', 'ablation',
            'cross-validation', 'testing', 'training', 'validation',
            'implementation', 'deployment', 'simulation', 'prototype'
        ]
        
        # Technology-specific validation patterns
        self.validation_patterns = [
            r'dataset size', r'training set', r'test set', r'validation set',
            r'epochs?', r'batch size', r'learning rate', r'hyperparameter',
            r'cross-validation', r'k-fold', r'confidence interval'
        ]
    ...
    def extract_features(self, text):
        ...
        features['accuracy_count'] = len(re.findall(self.accuracy_pattern, text))
        ...
        jargon_count = sum(text.lower().count(term) for term in self.tech_jargon)
        ...
        if 'results' in sections or 'evaluation' in sections:
            results_section = sections.get('results', sections.get('evaluation', ''))
            results_metrics = len(re.findall(self.accuracy_pattern, results_section))
            results_words = len(results_section.split()) + 1
            features['results_stat_density'] = results_metrics / results_words * 1000
        else:
        ...
        # 7. Technology performance claims
        performance_terms = ['performance', 'improvement', 'speedup', 'faster', 'efficient', 'better']
        ...
        if 'abstract' in sections and ('conclusion' in sections or 'future work' in sections):
            abstract = sections['abstract']
            conclusion = sections.get('conclusion', sections.get('future work', ''))
        ...
        features['chart_mentions'] = text.lower().count("chart") + text.lower().count("graph") + text.lower().count("plot")
        ...
        features['self_reference_count'] = (text.lower().count("our method") + 
                                        ...
                                        text.lower().count("our framework") +
                                        text.lower().count("our implementation"))
    ...
    def _extract_sections(self, text):
        ...
        section_markers = {
            ...
            'related': ['related work', 'literature review', 'background', 'prior work'],
            'methods': ['methodology', 'method', 'methods', 'approach', 'system design', 'architecture'],
            'implementation': ['implementation', 'system implementation', 'technical details'],
            'evaluation': ['evaluation', 'experiments', 'experimental results', 'experimental setup'],
    ...
    def _extract_key_claims(self, text):
        """Extract key claims from text using technology-specific terms"""
        claim_sentences = []
    ...
    def _compare_claims(self, claims1, claims2):
        """Compare claims for consistency using technology-specific terminology"""
...
class HierarchicalBiasPredictionModel(nn.Module):
    def __init__(self, bert_model_name, num_classes=3, dropout_rate=0.3, feature_dim=22):
        ...
        print("Loading BERT (reduced complexity)")
...
def train_hierarchical_model(train_dataloader, val_dataloader, model, device, 
    ...
    for epoch in range(epochs):
        ...
        if val_accuracy > best_val_accuracy:
            ...
            torch.save(best_model, 'best_hierarchical_technology_model.pt')
...
# Enhanced evaluation function with technology-specific metrics
def evaluate_hierarchical_model(test_dataloader, model, device, label_names, feature_names=None):
    """Evaluate model on test set with technology-specific analysis"""
    model.eval()
    all_preds = []
    all_labels = []
    ...
    try:
        ...
        plt.title('Classification Performance by Technology Research Bias Type')
        plt.tight_layout()
        plt.savefig('technology_classification_performance.png')
        plt.close()
    ...
    if len(present_labels) > 1:
        ...
        plt.ylabel('True')
        plt.title('Technology Research Bias Detection - Confusion Matrix')
        plt.savefig('technology_confusion_matrix.png')
        plt.close()
        ...
        plt.xlabel('Predicted')
        plt.ylabel('True')
        plt.title('Technology Research Bias Detection - Normalized Confusion Matrix')
        plt.savefig('technology_normalized_confusion_matrix.png')
        plt.close()
    else:
...
# Function to analyze technology-specific feature importance
def analyze_technology_feature_importance(model, feature_names):
    """
    Analyze which technology-specific features are most important for bias detection
    ...
    plt.title('Technology-Specific Feature Importance for Bias Detection')
    plt.tight_layout()
    plt.savefig('technology_feature_importance.png')
    plt.close()
    ...
    feature_categories = {
        ...
        'Statistical': ['p_value_count', 'signif_stars_count', 'accuracy_count', 
                       'percentage_count', 'results_stat_density'],
        'Linguistic': ['hedge_ratio', 'certainty_ratio'],
        'Technology-Specific': ['theory_term_ratio', 'jargon_term_ratio', 'performance_claim_ratio'],
    ...
    plt.title('Feature Category Importance for Technology Research Bias Detection')
    plt.tight_layout()
    plt.savefig('technology_feature_category_importance.png')
    plt.close()
...
def main():
    ...
    # Feature names for the technology-specific extractor (updated for 22 features)
    feature_names = [
    ...
    try:
        # Specify the path to your technology papers JSON file
        papers_df = load_papers_from_json('technology_papers.json')
        print(f"Loaded {len(papers_df)} technology papers")
        ...
        if len(papers_df) <= 3:  # Need at least 3 for train/val/test
            ...
            # Create dummy technology data
            papers_df = pd.DataFrame({
                'text': ['This is a sample technology research paper with state-of-the-art performance improvements and 95% accuracy.'] * 10,
    ...
    # Extract handcrafted features with technology-specific extractor
    print("Extracting technology-specific handcrafted features...")
        ...
        for i, label in enumerate(resampled_labels):
            if i < len(train_texts):  # Original sample
                ...
                if label in label_to_texts and label_to_texts[label]:
                    ...
                    synthetic_texts.append(f"Synthetic technology research paper text for class {label}")
    ...
    train_dataset = HierarchicalTechnologyPaperDataset(
    ...
    val_dataset = HierarchicalTechnologyPaperDataset(
    ...
    try:
        print("Using memory optimization settings for device")
    ...
    model = train_hierarchical_model(
        ...
        epochs=5,  # Increased to 5 epochs as requested
        accumulation_steps=4,  # Increased accumulation steps for small GPU
        lr=5e-5  # Increased learning rate for 4-5 epoch training
    )
    ...
    # Analyze technology-specific feature importance
    print("Analyzing technology-specific feature importance...")
    analyze_technology_feature_importance(model, feature_names)

    print("Technology bias detection analysis complete!")
