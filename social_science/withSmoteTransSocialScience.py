...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        if self.label_smoothing > 0:
            # Use built-in label smoothing from cross_entropy
...
def extract_sections(text):
    ...
    section_markers = {
        ...
        'literature': ['literature review', 'theoretical framework', 'background'],
        'methods': ['methodology', 'methods', 'research design', 'data and methods'],
    ...
    # Create a list of all markers with their keys to find their positions
    found_markers = []
    for section_key, markers in section_markers.items():
        for marker in markers:
            # Regex to find section headers that are on their own line
            for match in re.finditer(r'^' + re.escape(marker) + r'[:.\s]*$', text_lower, re.MULTILINE):
                found_markers.append({'key': section_key, 'pos': match.start()})
                break # Move to next section key once a marker is found

    # Sort markers by their position in the text
    found_markers.sort(key=lambda x: x['pos'])

    # Extract text between markers
    for i, marker_info in enumerate(found_markers):
        start_pos = marker_info['pos']
        # Find the start of the content after the marker
        content_start = text_lower.find('\n', start_pos) + 1
        
        # The end position is the start of the next marker, or the end of the text
        end_pos = found_markers[i+1]['pos'] if i + 1 < len(found_markers) else len(text)
        
        section_text = text[content_start:end_pos].strip()
        section_dict[marker_info['key']] = section_text
...
# Enhanced feature extractor for social science papers
class SocialScienceFeatureExtractor:
    """Extract features that might indicate bias in social science research papers"""
    ...
    def __init__(self):
        ...
        # Social science-specific hedge words
        self.hedge_words = [
        ...
        # Social science theory and framework references
        self.theory_terms = [
            'marx', 'weber', 'durkheim', 'bourdieu', 'foucault', 'habermas',
            'social theory', 'critical theory', 'post-structuralism', 'feminist theory',
            'constructivism', 'positivism', 'interpretivism', 'grounded theory',
            'social network analysis', 'qualitative analysis', 'quantitative analysis'
        ]
        
        # Social science-specific claim words
        self.claim_terms = [
            'impact', 'effect', 'influence', 'relationship', 'correlation',
            'causation', 'implication', 'social justice', 'inequality',
            'policy recommendation', 'social change', 'well-being', 'community',
            'identity', 'discourse', 'narrative'
        ]
        ...
        # M
        # ethodology and validation terms
        self.method_terms = [
            'survey', 'interview', 'case study', 'ethnography', 'experiment', 
            'observation', 'questionnaire', 'focus group', 'analysis', 'regression',
            'correlation', 'statistical analysis', 'qualitative', 'quantitative'
        ]
        
        # Social science-specific validation patterns
...
class FeatureFusionLayer(nn.Module):
    ...
    def forward(self, bert_embedding, handcrafted_features):
        ...
        if bert_proj.dim() == 2:
            ...
            # Query: bert_proj, Key: handcrafted_proj, Value: handcrafted_proj
        ...
        else: # Fallback for 3D tensors, though not used by current model
            # For 3D tensors, apply batch norm across the feature dimension
...
class HierarchicalBiasPredictionModel(nn.Module):
    def __init__(self, bert_model_name, num_classes=3, dropout_rate=0.3, feature_dim=22):
        ...
        # Use a gradient checkpointing wrapper to save memory during training
    ...
    def forward(self, input_ids, attention_mask, handcrafted_features):
        ...
        sent_att_weights[sent_mask.unsqueeze(-1) == 0] = -1e9 # Mask out padded sentences
        ...
        sect_att_weights[sect_mask.unsqueeze(-1) == 0] = -1e9 # Mask out padded sections
...
def train_hierarchical_model(train_dataloader, val_dataloader, model, device, 
    ...
    """Train the hierarchical model with a validation-based learning rate scheduler"""
    ...
    # On a balanced dataset (after SMOTE), class weights are not needed.
    class_weights = None
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, batch in enumerate(train_dataloader):
            ...
            # Use autocast only on GPU for performance benefits and to handle checkpointing warnings.
            # On CPU, autocast with float32 is not supported, so we run without it.
            if device.type == 'cuda':
        ...
        if len(train_dataloader) % accumulation_steps != 0:
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_grad_norm)
            optimizer.step()
            optimizer.zero_grad() # Zero out gradients after the final step
...
def evaluate_hierarchical_model(test_dataloader, model, device, label_names, feature_names=None):
    ...
    try:
        ...
        plt.title('Classification Performance by Social Science Bias Type')
        plt.tight_layout()
        plt.savefig('social_science_classification_performance.png')
        plt.close()
    ...
    if len(present_labels) > 1:
        ...
        plt.ylabel('True')
        plt.title('Social Science Bias Detection - Confusion Matrix')
        plt.savefig('social_science_confusion_matrix.png')
        plt.close()
        ...
        plt.xlabel('Predicted')
        plt.ylabel('True')
        plt.title('Social Science Bias Detection - Normalized Confusion Matrix')
        plt.savefig('social_science_normalized_confusion_matrix.png')
        plt.close()
    else:
...
def load_papers_from_json(json_file_path):
    """
    Load social science research papers from a JSON file.
    ...
    try:
        ...
        if isinstance(data, list):
            for i, paper in enumerate(data):
                ...
                for key in ['OverallBias', 'Overall Bias', 'Overall_Bias']:
                    if key in paper:
                        label = paper.get(key)
                        break
                # Try common alternative keys
                if label is None:
                    label = paper.get('bias_label', paper.get('bias_type', 
                ...
                if isinstance(label, str):
                    ...
                    label_map = {
                        'no bias': 0, 'nobias': 0,
                        'cognitive bias': 1, 'cognitivebias': 1,
                        'publication bias': 2, 'publicationbias': 2,
                        'selection bias': 2, 'selectionbias': 2,
                        'confirmation bias': 1,
                        'survivorship bias': 2
                    }
            ...
            # If it's a dictionary with IDs as keys
                ...
                for paper_id, paper_data in data.items():
                    if isinstance(paper_data, dict):
                        text = paper_data.get('Body', paper_data.get('text', ''))
                        # Prefer 'Overall Bias' keys
                        label = None
                        for key in ['OverallBias', 'Overall Bias', 'Overall_Bias']:
                            if key in paper_data:
                                ...
                                break
                        if label is None:
                            label = paper_data.get('bias_label', paper_data.get('bias_type', 
        ...
        print(f"Successfully loaded {len(df)} social science papers from {json_file_path}")
        return df
        ...
        print(f"Error loading social science papers from {json_file_path}: {str(e)}")
        return pd.DataFrame(columns=['text', 'label'])
...
def analyze_feature_importance(model, feature_names):
    ...
    plt.title('Social Science Feature Importance for Bias Detection')
    plt.tight_layout()
    plt.savefig('social_science_feature_importance.png')
    plt.close()
    
    # Plot feature importance by category
    # Group features by category for social science
    feature_categories = {
        ...
        'Section Analysis': ['abstract_claim_ratio', 'claim_consistency'], # Updated for social science
    ...
    sns.barplot(x='Average Importance', y='Category', data=category_df, palette='viridis')
    plt.title('Feature Category Importance for Social Science Bias Detection')
    plt.tight_layout()
    plt.savefig('social_science_feature_category_importance.png')
    plt.close()
...
def main():
    ...
    # Feature names for the social science-specific extractor
    feature_names = [
    ...
    try:
        # Specify the path to your social science papers JSON file
        papers_df = load_papers_from_json('social_science_papers.json')
        print(f"Loaded {len(papers_df)} social science papers")
    ...
    # Extract handcrafted features with social science-specific extractor
    ...
    try:
        ...
        # For synthetic samples, find the nearest original text
        from sklearn.neighbors import NearestNeighbors
        nn = NearestNeighbors(n_neighbors=1).fit(original_train_features)
        
        for i, label in enumerate(resampled_labels):
            if i < num_original_samples:  # Original sample
                synthetic_texts.append(train_texts[i])
            else:  # Synthetic sample
                # Find the nearest neighbor from the original data for this synthetic feature vector
                feature_vector = resampled_features[i].reshape(1, -1)
                _, indices = nn.kneighbors(feature_vector)
                synthetic_texts.append(train_texts[indices[0][0]])
    ...
    model = HierarchicalBiasPredictionModel(
        model_name, 
        num_classes=3,
        feature_dim=len(feature_names),  # 22 features for social science
    ...
    max_sections = 4      # Increased from 2
    max_sents = 8         # Increased from 4
    ...
    model = train_hierarchical_model(
        ...
        accumulation_steps=4,  # Increase accumulation for larger effective batch size
        lr=3e-5  # Lower learning rate is better for fine-tuning transformers
    )
    ...
    # Analyze social science-specific feature importance
    print("Analyzing social science-specific feature importance...")
    analyze_feature_importance(model, feature_names)
    
    print("Social science bias detection analysis complete!")
