...
# Enhanced feature extractor for economics papers
...
# Enhanced evaluation function with economics-specific metrics
def evaluate_hierarchical_model(test_dataloader, model, device, label_names, feature_names=None):
    """Evaluate model on test set with economics-specific analysis"""
    model.eval()
    all_preds = []
    all_labels = []
...
def analyze_economics_feature_importance(model, feature_names):
    """
    Analyze which economics-specific features are most important for bias detection
    ...
    plt.title('Economics-Specific Feature Importance for Bias Detection')
    plt.tight_layout()
    plt.savefig('economics_feature_importance.png')
    plt.close()
    ...
    feature_categories = {
        ...
        'Statistical': ['p_value_count', 'signif_stars_count', 'coefficient_count', 'results_p_value_density'],
        'Linguistic': ['hedge_ratio', 'certainty_ratio'],
        'Economics-Specific': ['theory_term_ratio', 'jargon_term_ratio', 'econometric_method_count'],
        'Section Analysis': ['abstract_claim_ratio', 'claim_consistency'],
        'Rigor': ['limitations_mentioned', 'robustness_check_ratio'],
    ...
    plt.title('Feature Category Importance for Economics Research Bias Detection')
    plt.tight_layout()
...
def main():
    ...
    try:
        ...
        papers_df = load_papers_from_json('economics_papers.json')
        ...
        if len(papers_df) <= 3:  # Need at least 3 for train/val/test
            ...
            papers_df = pd.DataFrame({
                'text': ['This is a sample economics research paper with p < 0.05 and significant results.'] * 10,
    ...
    # Extract handcrafted features with economics-specific extractor
        ...
        for i, label in enumerate(resampled_labels):
            if i < len(train_texts):  # Original sample
                ...
                if label in label_to_texts and label_to_texts[label]:
                    ...
                    synthetic_texts.append(f"Synthetic research paper text for class {label}")
    ...
    # Analyze economics-specific feature importance
    print("Analyzing economics-specific feature importance...")
    analyze_economics_feature_importance(model, feature_names)
